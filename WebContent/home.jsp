			...
			<c:forEach items="${imageList}" varStatus="status" var="image">
				<c:choose>
					<c:when test="${status.first or status.index % 4 eq 0}">
						<div class="col-xs-2 col-xs-ofset-2">
							<a href="#" class="thumbnail text-center">
								<img name="${image.name}" data="<fmt:formatDate value='${image.date}' pattern='yyyy-MM-dd HH:mm' />" style="width:140px;
									height:130px;"src="http://p9hndu2rh.bkt.clouddn.com/${image.url}">
					...
					<c:when test="${status.index % 4 eq 3 and not status.last}">
						<div class="col-xs-2">
							<a href="#" class="thunbnail text-center">
								<img name="${image.name}" data="<fmt:formatDate value='${image.date}' pattern='yyyy-MM-dd HH:mm' />" style="width:140px;
									height:130px;"src="http://p9hndu2rh.bkt.clouddn.com/${image.url}">
								<input class="pull-left" type="checkbox" value="${image.id}" url="${image.url}" />${image.name}
							</a>
					...
					<div class="modal-body">
						<form class="form-horizontal" role="form" id="form">
							<div class="form-group">
								...
								<div class="col-xs-4">
									<input type="text" id="form-control" id="image_name" name="image_name" />
								</div>
							</div>
							<div class="form-group">
